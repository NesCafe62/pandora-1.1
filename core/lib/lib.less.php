...
class lessc {
	...
	*/
	...
	public function checkedCompile($in, $out) {
		/* \console::log('src['.$in.']: '.date('H:i:s',filemtime($in)));
		if (is_file($out)) {
			\console::log('dst['.$out.']: '.date('H:i:s',filemtime($out)));
		}
		\console::log(date('H:i:s'),'time'); */
		if (!is_file($out) || filemtime($in) > filemtime($out)) {
			$this->compileFile($in, $out);
			// console::log(date('j M Y H:i:s',time()-60*60*3));
			// touch($out,time()-60*60*3);
	...
	protected function newFormatter() {
		$className = "lessc\lessc_formatter_lessjs";
		if (!empty($this->formatterName)) {
			if (!is_string($this->formatterName))
				return $this->formatterName;
			$className = "lessc\lessc_formatter_$this->formatterName";
		}
	...
	// compile file $in to file $out
	// returns true when it compiles, false otherwise
	public static function fcompile($in, $out, $less = null) {
		if ($less === null) {
			$less = new self;
		}
		return $less->compileFile($in, $out);
	}
...
class lessc_parser {
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array('lessc\lessc', 'preg_quote'),
	...
	protected function isDirective($dirname, $directives) {
		...
		$pattern = implode("|",
			array_map(array("lessc\lessc", "preg_quote"), $directives));
	...
	protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
		...
		$stop = array("'", '"', "@{", $end);
		$stop = array_map(array("lessc\lessc", "preg_quote"), $stop);
...
namespace {
	...
	class less {
		...
		public static function ccompile($lessfile) {
			$cssfile = preg_replace('/\.less$/','.css',$lessfile);
			if ($cssfile === $lessfile) {
				trigger_error('can not compile less file: "'.$lessfile.'"',E_USER_WARNING);
			} else {
				lessc\lessc::ccompile($lessfile,$cssfile);
			}
			return $cssfile;
		}
		
		public static function fcompile($lessfile) {
			$cssfile = preg_replace('/\.less$/','.css',$lessfile);
			if ($cssfile === $lessfile) {
				trigger_error('can not compile less file: "'.$lessfile.'"',E_USER_WARNING);
			} else {
				lessc\lessc::fcompile($lessfile,$cssfile);
			}
			return $cssfile;
		}
